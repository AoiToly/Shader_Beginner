这里记录了各个Example文件夹中展示的功能，介绍了Shader中的重点功能，具体实现细节可以查看具体Shader中的注释
...
包含两个SubShader，做了Shader的LOD

关于光照，实现了阴影的采样和投射，这部分实现细节可看Reference

使用了E7中的XRay效果

实现了溶解效果，包含边缘的颜色渐变效果

一二级SubShader的区别：一级Shader使用了Unity的Api；而二级Shader使用了计算更为简单的方式，将人物投射到y轴的一个平面上，并做偏移，但此方案容易穿帮
...
使用了GrabPass，此Pass非常耗，尽量少用

此示例里面有3个Shader，实现的功能相同，但在获取屏幕坐标的方式上有些区别

#### E5_UIGray

这是一个UI Shader，实现了两种效果

其一是UI的遮罩效果，有两种方案实现，其一是模板测试，其二是Rect2D的方形超过边缘裁剪

其二是图片变会的效果

#### E6_LightMode

实现了3种光照模型，Lambert、Phong和Blinn-Phong

注意ForwardBase 和 ForwardAdd的区别，ForwordBase用于渲染最重要的那一盏逐像素平行灯以及其他顶点和球谐灯，而ForwardAdd用于计算除了这盏逐像素平行灯之外的所有其他逐像素灯
...
使用菲涅尔反射计算物体的表面，并且增加了流动效果

#### E8_GlobalIllumination

这是个实现GI的Shader

包含Baked GI和Realtime GI，以及光照探针等内容
...
贴图的几种Shader使用方案，如WrapMode、Mipmap、物体法线纹理、Cube贴图、天空盒反射的颜色
